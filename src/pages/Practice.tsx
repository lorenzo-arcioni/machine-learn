...
type DifficultyLevel = keyof ExercisesData | 'Tutti';
...
export default function Practice() {
  ...
  const [activeLevel, setActiveLevel] = useState<DifficultyLevel>('Tutti');
  ...
  // Funzione per ottenere tutti gli esercizi in un singolo array
  const getAllExercises = (): Exercise[] => {
    return [
      ...currentExercisesData.Principiante,
      ...currentExercisesData.Intermedio,
      ...currentExercisesData.Avanzato
    ];
  };
  ...
  useEffect(() => {
    const loadAllExercises = async () => {
      ...
      if (exercisesData) {
        ...
        // Il tab "Tutti" è sempre disponibile se ci sono esercizi
        const totalExercises = exercisesData.Principiante.length + exercisesData.Intermedio.length + exercisesData.Avanzato.length;
        if (totalExercises > 0) {
          setActiveLevel('Tutti');
        }
      } else {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto py-8 px-4">
          ...
          <Tabs value={activeLevel} onValueChange={(val) => setActiveLevel(val as DifficultyLevel)} className="w-full">
            ...
            {/* Tab specifici per livello */}
            {(['Principiante', 'Intermedio', 'Avanzato'] as (keyof ExercisesData)[]).map(level => {
